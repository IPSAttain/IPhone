<?php
    class FindMyiPhoneModul extends IPSModule
    {
        ...
        public function ApplyChanges()
        {
            ...
            if (!$this->ReadPropertyBoolean('routing_information')) {
                $this->DeleteVars($Idents);
            }
            $Idents = array("fmip_dynmap");
            if (!$this->ReadPropertyBoolean('active_googlemap')) {
                $this->DeleteVars($Idents);
            }
            $Idents = array("fmip_googlemap", "fmip_googlemap_zoom");
            if (!$this->ReadPropertyBoolean('static_googlemap')) {
                $this->DeleteVars($Idents);
            }
        }
        ...
        protected function ProceedData($data)
        {
            ...
            foreach ($devices as $device) {
                if ($this->ReadPropertyString('DeviceID') == $device['id']) {
                    ...
                    $Ident = 'batteryStatus';
                    if (isset($device[$Ident])) {
                        $this->SaveData('fmip_' . $Ident, $device[$Ident], $this->Translate('Battery'), '', 'String', 3);
                    }
                    $Ident = 'name';
                    if (isset($device[$Ident])) {
                        $this->SaveData('fmip_' . $Ident, $device[$Ident], $this->Translate('Name'), '', 'String', 1);
                    }
                    $Ident = 'lowPowerMode';
                    if (isset($device[$Ident])) {
                        $this->SaveData('fmip_' . $Ident, $device[$Ident], $this->Translate('Low Power Mode'), '~Switch', 'Bool', 5);
                    }
                    $Ident = 'location';
        ...
        {
            if ($VarType == "Bool") {
                $this->RegisterVariableBoolean($Ident, $this->Translate($Name), $Profil, $TreePosition);
            }
            if ($VarType == "Integer") {
                $this->RegisterVariableInteger($Ident, $this->Translate($Name), $Profil, $TreePosition);
            }
            if ($VarType == "Float") {
                $this->RegisterVariableFloat($Ident, $this->Translate($Name), $Profil, $TreePosition);
            }
            if ($VarType == "String") {
                $this->RegisterVariableString($Ident, $this->Translate($Name), $Profil, $TreePosition);
            }
