...
class FindMyiPhone {
	...
	public function refresh_client() {
        ...
        if (property_exists(json_decode($this->make_request('refreshClient', $post_data)), 'userInfo'))
        {
	...
	public function notify_when_found($device_id, $notify = true) {
		if(!is_string($device_id)) throw new FindMyiPhoneException('Expected $device_id to be a string');
		if(!is_bool($notify)) throw new FindMyiPhoneException('Expected $notify to be a boolean');
